...
# Load the embedding model ONCE at module level so it persists across
# jobs when using SimpleWorker (no fork → no model reload per job).
...
async def process_chat_pipeline(job_id: str, s3_key: str, reciever_data: dict):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        embedded = await _embedder.embed_pairs(parsed)
